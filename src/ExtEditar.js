...
const ExtEditar = () => {
  ...
  const editado = {
    ...
    ultRet: ultRet,
    avaria: avaria,
    vistoria: { check: true, stamp: Date.now() },
  };
  ...
  return (
    <div className={styles.novoExtContainer}>
      <h2>
        <i className="fa-regular fa-pen-to-square" /> Editar Extintor
      </h2>
      ...
      <fieldset className={styles.fieldset}>
        ...
        <Select
          ...
          onBlur={({ currentTarget }) => handleBlur(currentTarget)}
          onFocus={({ currentTarget }) => handleFocus(currentTarget)}
          optionDisabledValue={'Tipo'}
          selClass={styles.select}
          optClass={styles.option}
          selectValorInicial={tipo}
      ...
      <fieldset className={styles.fieldset}>
        ...
        <Select
          ...
          selectOnChange={({ currentTarget }) => handleSelect(currentTarget, 2)}
      ...
      <fieldset className={styles.fieldset}>
        ...
        <div className={styles.wrapperSelectRecarga}>
          <Select
            onBlur={({ currentTarget }) => handleBlurRecarga(currentTarget)}
            onFocus={({ currentTarget }) => handleFocusRecarga(currentTarget)}
            className={styles.selectRec}
            optClass={styles.option}
            selectValorInicial={mes}
          ...
          <Select
            ...
            className={styles.selectRec}
            optClass={styles.option}
            selectValorInicial={ano}
            selectOnChange={({ currentTarget }) =>
              handleSelect(currentTarget, 4)
            }
      ...
      <fieldset className={`${styles.fieldset} ${styles.textareaWrapper}`}>
        <textarea
          ...
          id="hdAvariasTxtArea"
          value={avaria}
          onChange={({ target }) => setAvaria(target.value)}
        ></textarea>
      </fieldset>
