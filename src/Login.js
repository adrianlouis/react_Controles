...
{/* //CONFSENHA */}
                ...
                <span className='regInvalido'>{regMsg.nome && regInput.nome !== '' ? 'Nome já cadastrado. ':''}</span>
                <span className='regInvalido'>{regMsg.email && regInput.email !== '' ? 'Email já cadastrado. ':''}</span>

                {!regOk.senha && regInput.senha !== '' && <><span className='regInvalido'>{regexRegSenha.num ? '':'Senha precisa de número'}</span>
                <span className='regInvalido'>{regInput.senha && regexRegSenha.esp?'':'Senha precisa de dígito especial'}</span>
                <span className='regInvalido'>{regInput.senha && regexRegSenha.tamanho?'':'Senha precisa ao menos 8 dígitos'}</span></>}
