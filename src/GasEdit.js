...
const GasEdit = () => {
    ...
    const [medicaoNova, setMedicaoNova] = React.useState({loja:'', medicao:''})
    const [novosItens, setNovosItens] = React.useState([])

    function addMedicaoDeLoja(){
        if (document.querySelector('#gasMedicao').value === '' && document.querySelector('#numeroLoja').value === ''){
            return
        }else{
            setObjEditavel({...objEditavel, medicao:[...objEditavel.medicao, medicaoNova]})
  ...
  return (
    <div>
        ...
        {objEditavel.medicao.map((m, ind)=>{
            return <div id='teste' key={'item'+ind} className={styles.wrapperEdicaoGas}>
            <GasEdt loja={m.loja} valor={m.medicao} onDel={({currentTarget})=>deletar(m.loja, currentTarget, ind)} />
        </div>
            ...
            <div className={styles.addGasLine}>
                ...
                <div className='gasAcoes'>
                    <i className="fa-solid fa-square-plus" style={medicaoNova.loja === '' && medicaoNova.medicao === '' ? {color:'rgb(121,121,121)', cursor:'not-allowed'} : {color:'rgb(166,243,166)', cursor:'pointer'}} onClick={()=>addMedicaoDeLoja()}></i>
                </div>
            </div>

        <div className={styles.editActBtn}>
            <span onClick={()=>navigate('/home/gas')} ><i className="fa-solid fa-angle-left"/> Cancelar</span>
            <span onClick={()=>salvar()}><i className="fa-regular fa-floppy-disk"/> Salvar</span>
        </div>
