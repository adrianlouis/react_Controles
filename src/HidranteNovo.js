...
//   </div>
  <div className={styles.novoExtContainer} >
    <h2><i className="fa-solid fa-file-pen"/> Novo Hidrante</h2>
    ...
    <fieldset className={styles.fieldset} >
      ...
      <Select value={local} onBlur={({currentTarget})=>handleBlur(currentTarget)} onFocus={({currentTarget})=>handleFocus(currentTarget)} selClass={styles.select} optClass={styles.option} selectValorInicial={'Local'} selectOnChange={({currentTarget})=>handleSelect(currentTarget, 1)} optionDisabledValue='Local' options={['Subsolo', 'Térreo', 'Brigada', '2º Pav A', '2º Pav B', '2º Pav C', '3º Pav A', '3º Pav B', '3º Pav C', '4º Pav A', '4º Pav B', '4º Pav C', 'CMI']} />
    </fieldset>

    <fieldset className={styles.fieldset} >
      <i className="fa-solid fa-store" />
      <Select onBlur={({currentTarget})=>handleBlur(currentTarget)} onFocus={({currentTarget})=>handleFocus(currentTarget)} optClass={styles.option}
        selectValorInicial={abrigo}
        options={["Ok", "Nok"]}
        selectOnChange={({target})=>setAbrigo(target.value)}
    ...
    <fieldset className={`${styles.fieldset} ${styles.pecasHidrante}`} >
      <div>
        ...
        <p><i className="fa-solid fa-wrench"/> Peças presentes no hidrante</p>
    ...
    <fieldset className={styles.fieldset} >
      ...
      <div className={styles.wrapperSelectRecarga}>
          <Select onFocus={({currentTarget})=>handleFocusReteste(currentTarget)} onBlur={({currentTarget})=>handleBlurReteste(currentTarget)} className={styles.selectRec} optClass={styles.option} value={hdValidade.mes} selectOnChange={({currentTarget})=>handleSelect(currentTarget, 2)} optionDisabledValue='Mês' options={['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']} />
          <Select onFocus={({currentTarget})=>handleFocusReteste(currentTarget)} onBlur={({currentTarget})=>handleBlurReteste(currentTarget)} className={styles.selectRec} optClass={styles.option} value={hdValidade.ano} selectOnChange={({currentTarget})=>handleSelect(currentTarget, 3)} optionDisabledValue='Ano' options={[anoAtual-3, anoAtual-2, anoAtual-1, anoAtual, anoAtual+1]} />
      </div>
    </fieldset>

    <fieldset className={styles.fieldset} >
      <textarea spellCheck='false' placeholder='Avarias . . .' onBlur={({currentTarget})=>handleBlur(currentTarget)} onFocus={({currentTarget})=>handleFocus(currentTarget)} id='hdAvariasTxtArea' value={avarias} onChange={({target})=>setAvarias(target.value)}></textarea>
    </fieldset>
    ...
    <div className={styles.actionBtnsCreateWrapper}>
        ...
        <span onClick={()=>salvarNovoHd(context.userLogado.id)} ><i className="fa-regular fa-floppy-disk" /> Salvar</span>
    </div> 
