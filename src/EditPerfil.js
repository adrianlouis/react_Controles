...
import { GET_POSTS } from './funcoes/Api';
...
const EditPerfil = () => {
  ...
  const [excluirFotos, setExcluirFotos] = React.useState({
    foto: false,
    wpp: false,
  });
  ...
  const [backupFoto, setBackupFoto] = React.useState({
    foto: context.imgTemp.foto,
    fCrop: context.imgTemp.fCrop,
    wpp: context.imgTemp.wpp,
    wCrop: context.imgTemp.wCrop,
  });
  ...
  async function handleCrop() {
    ...
    fotoDoPerfil.onload = () => {
      ...
      setTemps({
        ...temps,
        fCrop: [cut.x, cut.y, cut.width, cut.height, 0, 0, 80, 80],
      });
      context.setImgTemp({
        ...context.imgTemp,
        fCrop: [cut.x, cut.y, cut.width, cut.height, 0, 0, 80, 80],
  ...
  async function handleSave() {
    ...
    if (context.imgTemp.fileFoto && excluirFotos.foto === false) {
      ...
      context.setUserLogado({
        ...context.userLogado,
        perfil: { ...context.userLogado.perfil, foto: '', fotoCrop: '' },
      });
      context.setImgTemp({ ...context.imgTemp, foto: false, fCrop: false });
    }
    ...
    updateBd(context.userLogado.id, {
      perfil: {
        ...context.userLogado.perfil,
        ...saveProf,
        fotoCrop: context.imgTemp.fCrop,
        foto: context.imgTemp.foto,
        wallpaper: context.imgTemp.wpp,
        wallpaperCrop: context.imgTemp.wCrop,
      },
    });
    context.setUserLogado((prev) => {
      return { ...prev, perfil: { ...prev.perfil, ...saveProf } };
    });
  ...
  async function back() {
    context.setImgTemp({
      foto: backupFoto.foto,
      fCrop: backupFoto.fCrop,
      wpp: backupFoto.wpp,
      wCrop: backupFoto.wCrop,
    });
    setEditado('');
  ...
  React.useEffect(() => {
    document
      .querySelector(':root')
      .style.setProperty('--corEscolhida', rootCor);
  ...
  return (
    <div>
      {modal === 1 && (
        <div
          id="modal"
          className="perfilEditModal"
          onClick={(e) => handleModal(e)}
        >
          <div>
            <p onClick={() => document.querySelector('#editFoto').click()}>
              Mudar foto
            </p>
      ...
      {modal === 2 && (
        ...
        >
          <div>
            <p
              onClick={() =>
                document.querySelector('#inputFileWallpaper').click()
              }
            >
      ...
      {modal === 4 && (
        <div className="perfilEditModalCrop">
          <div className={styles.wrapperCrop}>
            <Cropper
              cropShape="rect"
              showGrid={false}
              image={context.imgTemp.wpp}
              crop={crop}
              zoom={zoom}
              aspect={3 / 1}
          ...
          <div className={styles.wrapperCropBtns}>
            <button onClick={() => cancelWppCrop()}>
              <i className="fa-solid fa-xmark"></i> Cancelar
            </button>
            <button onClick={() => handleWppCrop()}>
      ...
      <div id="perfil" className="perfil">
        ...
        <input
          type="file"
          id="inputFileWallpaper"
          style={{ display: 'none' }}
          onChange={handleWallpaper}
        ></input>
        <i
          id="iconWallpaperEdit"
        ...
        <div className="fotoPerfilWrapper" onClick={() => setModal(1)}>
          ...
          {!context.imgTemp.foto && (
            <i id="userSemFoto" className="fa-solid fa-user"></i>
          )}
        </div>
        ...
        <div className="dadosPerfil">
          <div className="editInputWrapper">
            ...
            <input
              id="editNome"
              className="inputEditProfile editNome"
              placeholder={context.fbAuth.displayName}
              value={editado.nome}
              onChange={({ target, currentTarget }) =>
                checkarNome(currentTarget, target.value, context.userLogado.id)
              }
              onFocus={({ target }) => inputFocus(target)}
              onBlur={({ target }) => inputBlur(target)}
            />
          ...
          <div className="editInputWrapper">
            ...
            <input
              id="editNick"
              className="inputEditProfile editNick"
              value={editado.nick}
              onChange={({ target }) =>
                setEditado({ ...editado, nick: target.value })
              }
              onFocus={({ target }) => inputFocus(target)}
              onBlur={({ target }) => inputBlur(target)}
            />
            <p className="editNickUsado">
          ...
          <div className="editInputWrapper">
            ...
            <textarea
              ...
              value={editado.quote}
              onChange={({ target }) =>
                setEditado({ ...editado, quote: target.value })
              }
              onFocus={({ target }) => inputFocus(target)}
              onBlur={({ target }) => inputBlur(target)}
            ></textarea>
          ...
          <div className={styles.wrapCores}>
            ...
            <input
              ...
              type="color"
              onChange={({ target }) => setRootCor(target.value)}
              value={rootCor}
            ></input>
          </div>
        </div>
      </div>
      <p id="botaoEditarCancel" onClick={() => back()}>
        Cancelar mudanças...
      </p>
    </div>
  );
